...
declare global {
  interface Window {
    __CORTEX_MODE__?: 'sidebar' | 'tab' | 'accounts' | 'rules' | 'analytics' | 'agent';
  }
}
...
function SidebarApp() {
  ...
  return (
    <div class="app">
      {accounts !== undefined && accounts.filter((a) => !a.reviewOnly).length === 0 && (
        <div class="setup-banner">
          <div class="setup-banner-text">Connect Claude, Codex or Grok — one click per subscription.</div>
          <button class="run-btn send" onClick={() => vscode.postMessage({ kind: 'openAccounts' })}>
            <IconPlus size={11} /> Open links
          </button>
        </div>
      )}
      <HistoryList
...
function ChatApp() {
  ...
  const [pinnedTarget, setPinnedTarget] = useState<PinnedTarget | undefined>();
    ...
    <div class="app chat-tab">
      ...
      >
        <div class="chat-column">
          <Transcript
            ...
            onRate={(messageId, poor) => vscode.postMessage({ kind: 'rateAnswer', messageId, poor })}
            onOpenFile={(path) => vscode.postMessage({ kind: 'openWorkspaceFile', path })}
            onOpenCode={(text, language) => vscode.postMessage({ kind: 'openCode', text, language })}
          />
          <div ref={bottomRef} />
        </div>
      </div>
      <div class="chat-column composer-holder">
        ...
        <Composer
          ...
          onPinnedTarget={(target) => {
            setPinnedTarget(target);
            vscode.postMessage({ kind: 'setPinnedTarget', target });
          }}
          onModeChange={({ ask, ...modes }) => {
            ...
            if (ask !== undefined) setAskPermission(ask);
            if (modes.permissionMode) setPermissionMode(modes.permissionMode);
            if (modes.routingMode) setRoutingMode(modes.routingMode);
            if (modes.permissionMode || modes.routingMode || ask !== undefined) {
              vscode.postMessage({ kind: 'setModes', ...modes, ask });
            }
          }}
        />
      </div>
