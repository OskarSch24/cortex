...
export class WorkspaceContext extends ProjectScoped<WorkspaceContext> {
  ...
  editorContext(): EditorContext {
    ...
    if (editor && editor.document.uri.scheme === 'file' && !rel(editor.document.uri).startsWith('..')) {
  ...
  private shapingFor(task: TaskRequest, checks: VerifyCommand[]): string[] {
    ...
    return shapeTask({
      ...
      activeFile: (task.editorSnapshot ?? this.snapshot.editor).activeFile,
  ...
  buildFor(
    ...
    try {
      return briefSections({
        provider,
        editor: task.editorSnapshot ?? this.snapshot.editor,
