...
export interface ToolStep {
  ...
  path?: string;
  action?: ToolAction;
  /** Lines written and replaced — `+68 -0` behind the file name. */
  added?: number;
  removed?: number;
}
...
/**
 * Die Dateien, die der Agent in diesem Chat angefasst hat — geschriebene und
 * gelesene, jede einmal, in der Reihenfolge des Chats.
 *
 * Grundlage der Übersichtskarte, und zwar unabhängig von git: nicht jedes
 * Projekt ist ein Repository, und ein Ordner voller Dokumente ist es fast nie.
 * Was der Agent angefasst hat, weiß Cortex aus seinen eigenen Werkzeugaufrufen.
 *
 * Hat er eine Datei erst gelesen und später geschrieben, gilt geschrieben: das
 * ist die stärkere Aussage über die Datei.
 */
...
export function touchedFiles(items: TranscriptItem[]): TouchedFile[] {
  const order: string[] = [];
  const mode = new Map<string, 'write' | 'read'>();
  for (const item of items) {
    ...
    for (const segment of item.segments) {
      ...
      for (const step of segment.steps) {
        ...
        if (!path) continue;
        const wrote = step.action === 'write' || step.action === 'edit';
        if (!wrote && step.action !== 'read') continue;
        if (!mode.has(path)) order.push(path);
        if (wrote || !mode.has(path)) mode.set(path, wrote ? 'write' : 'read');
      }
    }
  }
  return order.map(path => ({ path, mode: mode.get(path)! }));
}

/** Nur die geschriebenen — für alles, was sich auf echte Änderungen bezieht. */
export function editedPaths(items: TranscriptItem[]): string[] {
  return touchedFiles(items).filter(f => f.mode === 'write').map(f => f.path);
}
...
export type TranscriptItem =
  ...
  | {
      ...
      ruleId?: string;
      reason?: string;
      /** When the answer finished, for the line under it. */
      at?: number;
      /** The user marked this answer poor. */
      ratedPoor?: boolean;
      /** Its file changes were undone from the change card. */
      reverted?: boolean;
      canRevert?: boolean;
      revertReason?: string;
      costUsd?: number;
      ...
      metered?: boolean;
      durationMs?: number;
      /** Was ein Bildwerkzeug in diesem Auftrag geschrieben hat, in der Reihenfolge des Eintreffens. */
  ...
  | { kind: 'memories'; bereiche: string[]; treffer: import('./protocol.js').MemoryHitDto[] }
  | { kind: 'error'; text: string; messageId?: string; recovery?: 'install-claude' | 'reconnect-grok' };
...
export function applyHostMessage(items: TranscriptItem[], msg: HostToWebview): TranscriptItem[] {
  ...
  switch (msg.kind) {
    ...
    case 'memories': {
      next.push({ kind: 'memories', bereiche: msg.bereiche, treffer: msg.treffer });
      break;
    }
    case 'failover': {
    ...
    case 'rated': {
      ...
      if (i !== -1) {
        const item = next[i] as Extract<TranscriptItem, { kind: 'assistant' }>;
        next[i] = { ...item, ratedPoor: msg.poor };
      }
      break;
    }
    case 'revertState': {
      const i = lastAssistant(msg.messageId);
      if (i !== -1) next[i] = { ...next[i] as Extract<TranscriptItem, { kind: 'assistant' }>, canRevert: msg.available, revertReason: msg.reason };
      break;
    }
    case 'reverted': {
      const i = lastAssistant(msg.messageId);
      if (i !== -1) next[i] = { ...(next[i] as Extract<TranscriptItem, { kind: 'assistant' }>), reverted: true };
      break;
    }
    case 'stopped': {
    ...
    case 'error': {
      ...
      next.push({ kind: 'error', text: msg.message, messageId: msg.messageId, ...(msg.recovery ? { recovery: msg.recovery } : {}) });
      break;
    }
    default:
      // Nachrichten, die den Verlauf nicht betreffen — Kontostände, Projekte,
      // Ansichtszustand. Dieselbe Liste zurückgeben, sonst zeichnet das Fenster
      // den ganzen Chat bei jeder davon neu.
      return items;
  }
  return next;
}
