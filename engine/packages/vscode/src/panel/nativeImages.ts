...
export async function resizeGeneratedImage(source: string, width: number, height: number): Promise<string> {
  ...
  const destination = join(folder, `${randomUUID()}.png`);
  try {
    await run('/usr/bin/sips', ['-s', 'format', 'png', '-z', String(height), String(width), source, '--out', destination], { timeout: 30_000 });
    return destination;
  } catch (error) {
    await rm(destination, { force: true });
    const format = extname(source).slice(1).toUpperCase() || 'Bild';
    throw new Error(`Diese macOS-Version konnte die ${format}-Datei nicht in PNG umwandeln und skalieren. Exportiere sie zuerst als PNG oder JPEG und versuche es erneut. Das Original bleibt unverändert.`, { cause: error });
  }
}
...
export async function copyGeneratedImage(source: string): Promise<void> {
  ...
  const folder = join(dirname(source), 'cortex-edits');
  await mkdir(folder, { recursive: true });
  const png = join(folder, `clipboard-${randomUUID()}.png`);
  try {
...
export async function removeGeneratedImageBackground(source: string, helperPath: string, signal?: AbortSignal): Promise<string> {
  ...
  const destination = join(folder, `${randomUUID()}.png`);
  try {
    await run(helperPath, ['remove-background', source, destination], { timeout: 90_000, maxBuffer: 1024 * 1024, signal });
    return destination;
  } catch (error) {
    await rm(destination, { force: true });
    const failure = error as NodeJS.ErrnoException & { stderr?: string; killed?: boolean };
    throw new Error(failure.code === 'ENOENT' ? 'Der lokale Bildhelfer fehlt. Bitte Cortex neu bauen oder aktualisieren.'
