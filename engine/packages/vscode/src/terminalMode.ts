...
export async function openInTerminal(
  ...
  if (all.length === 0) {
    void vscode.window.showWarningMessage('cortex: no accounts yet — run "cortex: Add Account" first.');
    return;
  }
  ...
  const pick = await vscode.window.showQuickPick(
    ...
    { title: 'cortex: open an interactive session', placeHolder: 'Which account?' },
  );
  if (!pick) return;
  ...
  const terminal = vscode.window.createTerminal({
    name: `cortex: ${formatTarget(target)}`,
