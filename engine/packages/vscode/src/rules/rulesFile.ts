...
export class RulesManager implements vscode.Disposable {
  ...
  private diagnostics = vscode.languages.createDiagnosticCollection('cortex');
  ...
  constructor(private readonly approvals?: ApprovalStore) {
    ...
    const patterns: Array<string | vscode.RelativePattern> = [
      '**/.cortex/{rules,commands}.json',
      '**/.usrouter/{rules,commands}.json',
      new vscode.RelativePattern(
        vscode.Uri.file(join(homedir(), '.cortex')),
        '{rules,commands}.json',
      ),
      new vscode.RelativePattern(
  ...
  /** Workspace rules file wins; falls back to ~/.cortex, then legacy .usrouter paths. */
  locate(): { path: string; exists: boolean } {
    const ws = vscode.workspace.workspaceFolders?.[0];
    const candidates: string[] = [];
    if (ws) {
      candidates.push(join(ws.uri.fsPath, '.cortex', 'rules.json'));
      candidates.push(join(homedir(), '.cortex', 'rules.json'));
  ...
  private commandCandidates(): Array<{ path: string; fromWorkspace: boolean }> {
    ...
    for (const dir of ['.cortex', '.usrouter']) {
      if (inWorkspace)
  ...
  async openOrCreateCommands(): Promise<void> {
    ...
    const path =
      existing ??
      (ws ? join(ws.uri.fsPath, '.cortex', 'commands.json') : join(homedir(), '.cortex', 'commands.json'));
  ...
  load(): void {
    ...
    if (parsed.ok) {
      ...
      this.lastError = undefined;
      // A model the provider does not offer is not a broken file — the run
      // still happens, on the CLI's default. Say so where the rule is written,
      // instead of letting it surface as a surprise mid-run.
      const uri = vscode.Uri.file(path);
      ...
      this.diagnostics.set(uri, [
        new vscode.Diagnostic(
          new vscode.Range(0, 0, 0, 1),
          `cortex rules invalid (falling back to priority order): ${parsed.error}`,
  ...
  private async write(rules: RulesFile): Promise<void> {
    ...
    if (this.isUnparsed()) {
      ...
      const pick = await vscode.window.showErrorMessage(
        `cortex did not save: ${path} does not parse, and writing over it would discard the rules still in it. Fix the JSON first.`,
        open,
      );
