...
export class MetricsStore {
  ...
  /** The user said this answer was poor — or took it back. */
  async markRatedPoor(id: string, poor: boolean): Promise<void> {
    const metrics = this.all();
    const target = metrics.find((m) => m.id === id);
    if (!target || Boolean(target.ratedPoor) === poor) return;
    target.ratedPoor = poor || undefined;
    await this.ctx.globalState.update(KEY, metrics);
    this.emitter.fire();
  }
