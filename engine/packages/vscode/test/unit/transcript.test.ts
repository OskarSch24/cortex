...
describe('transcript reducer', () => {
  it('preserves explicitly attached source paths through compaction and replay', () => {
    const log: HostToWebview[] = [{ kind: 'userEcho', text: 'Review the screenshot', attachments: ['/tmp/screenshot.png'] }];
    expect(reduceTranscript(compactLog(log))).toEqual([{ kind: 'user', text: 'Review the screenshot', attachments: ['/tmp/screenshot.png'] }]);
  });

  it('carries the send time of a message, and survives one that predates it', () => {
    const sent = Date.UTC(2026, 8, 8, 19, 44);
    const log: HostToWebview[] = [
      { kind: 'userEcho', text: 'neu', at: sent },
      { kind: 'userEcho', text: 'aus einem alten Protokoll' },
    ];
    expect(reduceTranscript(compactLog(log))).toEqual([
      { kind: 'user', text: 'neu', at: sent },
      { kind: 'user', text: 'aus einem alten Protokoll' },
    ]);
  });
  ...
  it('builds a happy-path turn: echo → routing → deltas → done', () => {
    ...
    expect(applyHostMessage(items, { kind: 'conversationReady' })).toEqual(items);
