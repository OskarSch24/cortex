...
const RULES_PATH = join(homedir(), '.cortex', 'rules.json');
...
describe('RulesManager — a file that does not parse', () => {
  ...
  it('refuses to save over it, because the rules in it are still there', async () => {
    ...
    await manager.saveDefaultChain([{ provider: 'copilot', account: 'main' }]);
  ...
  it('saves again once the file parses', async () => {
    ...
    expect(written()).toMatchObject({
      defaultChain: [{ provider: 'copilot', account: 'main' }],
...
describe('RulesManager — slash commands defined by the open workspace', () => {
  ...
  const WS_COMMANDS = join(WS, '.cortex', 'commands.json');
  const HOME_COMMANDS = join(homedir(), '.cortex', 'commands.json');
