...
describe('orchestrator failover', () => {
  it('keeps common instructions on cold failover and visits a circular chain only once', async () => {
    const fake = new FakeAdapter('claude');
    fake.script('claude-a', [{ type: 'limit', scope: 'session', raw: 'limit' }]);
    fake.script('claude-b', [{ type: 'limit', scope: 'session', raw: 'limit' }]);
    const adapters = new AdapterRegistry(); adapters.register(fake);
    const orchestrator = new Orchestrator({ adapters, quota: new QuotaTracker(), sessions: new SessionStore(),
      getRules: () => ({ ...rules, defaultChain: [...rules.defaultChain, rules.defaultChain[0]!] }),
      getAccounts: () => accounts, resolveAccount: async target => accounts.find(account => account.label === target.account),
      getBrief: () => [{ id: 'common', title: 'Gemeinsam', body: 'Keep project instructions on every account.' }],
      getProviderBrief: () => ({ text: 'Standing shared instruction.', lineIds: [] }),
    });
    await collect(orchestrator.run({ ...task, routingMode: 'manual' }, new AbortController().signal));
    expect(fake.runs.map(run => run.accountId)).toEqual(['claude-a', 'claude-b']);
    for (const run of fake.runs) { expect(run.prompt).toContain('Keep project instructions'); expect(run.systemBrief).toBe('Standing shared instruction.'); }
  });
  it('passes explicit effort through to the adapter instead of automatic sizing', async () => {
    const fake = new FakeAdapter('claude');
    fake.script('claude-a', [{ type: 'result', text: 'ok' }]);
    const run = vi.spyOn(fake, 'run');
    const { orchestrator } = setup(fake);
    await collect(orchestrator.run({ ...task, effort: 'max' }, new AbortController().signal));
    expect(run.mock.calls[0]![0].effort).toBe('max');
  });

  it('passes the task approval choice to the provider', async () => {
    const fake = new FakeAdapter('claude');
    fake.script('claude-a', [{ type: 'result', text: 'ok' }]);
    const run = vi.spyOn(fake, 'run');
    const { orchestrator } = setup(fake);
    await collect(orchestrator.run({ ...task, permissionMode: 'edits', askPermission: true }, new AbortController().signal));
    expect(run.mock.calls[0]![0]).toMatchObject({ permissionMode: 'edits', askPermission: true });
  });

  it.each([undefined, {}, { selected: { command: 'node', args: ['mcp.mjs'] } }] as TaskRequest['mcpServers'][])('preserves the exact per-run MCP selection through routing', async mcpServers => {
    const fake = new FakeAdapter('claude');
    fake.script('claude-a', [{ type: 'result', text: 'ok' }]);
    const run = vi.spyOn(fake, 'run');
    const { orchestrator } = setup(fake);
    await collect(orchestrator.run({ ...task, mcpServers }, new AbortController().signal));
    expect(run.mock.calls[0]![0].mcpServers).toBe(mcpServers);
  });
