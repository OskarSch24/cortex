...
describe('permission vocabulary', () => {
  ...
  it('remembers "always" per command, not for every command', () => {
    ...
    memory.remember(git);
    expect(memory.isAllowed({ ...git, id: '3' })).toBe(true);
    expect(memory.isAllowed({ ...git, id: '4', detail: 'git log' })).toBe(false);
    expect(memory.isAllowed({ ...git, detail: 'git reset --hard' })).toBe(false);
    expect(memory.isAllowed({ ...git, detail: 'git status; rm -rf /tmp/example' })).toBe(false);
    expect(memory.isAllowed(rm), 'allowing git must never allow rm').toBe(false);
    memory.remember({ id: 'empty', kind: 'command', title: 'run command' });
    expect(memory.isAllowed({ id: 'other', kind: 'command', title: 'run command' })).toBe(false);
    const edit: PermissionRequest = { id: 'edit', kind: 'edit', tool: 'Edit', title: 'Edit', path: 'a.ts', detail: 'same patch' };
    memory.remember(edit);
    expect(memory.isAllowed({ ...edit, path: 'b.ts' })).toBe(false);
    expect(memory.isAllowed({ ...edit, tool: 'Delete' })).toBe(false);
  });
});
...
describe('the permission gate', () => {
  ...
  it('accepts a synchronous host response without losing it', async () => {
    const gate = new PermissionGate({ mode: 'edits', ask: true, emit: (r) => gate.respond(r.id, { outcome: 'deny', reason: 'Use a safer command.' }) });
    expect(await gate.ask(request())).toEqual({ outcome: 'deny', reason: 'Use a safer command.' });
  });
