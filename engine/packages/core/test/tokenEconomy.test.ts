...
describe('what a session was told survives a reload', () => {
  const target = { provider: 'claude' as const, account: 'a' };
  const state = { editor: 'src/a.ts' };

  it('comes back with the session it belongs to', () => {
    const before = new SessionStore();
    before.setNativeSession('c', target, '/tmp', 'sid-1');
    before.rememberTaskBrief('c', target, '/tmp', state, { sentFull: false, contextTokens: 30_000 });
    const after = new SessionStore();
    after.restoreNative(before.serializeNative());
    after.restoreTaskBriefs(JSON.parse(JSON.stringify(before.serializeTaskBriefs())));
    expect(after.taskBriefState('c', target, '/tmp')).toEqual(state);
  });

  it('does not restate unchanged standing instructions after a reload', () => {
    const before = new SessionStore();
    before.setNativeSession('c', target, '/tmp', 'sid-1');
    before.rememberBrief('c', target, '/tmp', 'Always answer in German.');
    const after = new SessionStore();
    after.restoreNative(before.serializeNative());
    after.restoreBriefs(JSON.parse(JSON.stringify(before.serializeBriefs())));
    expect(after.briefChanged('c', target, '/tmp', 'Always answer in German.')).toBe(false);
    expect(after.briefChanged('c', target, '/tmp', 'Always answer in English.')).toBe(true);
    expect(JSON.stringify(before.serializeBriefs())).not.toContain('German');
  });

  it('is not kept for a session nobody remembers', () => {
    const sessions = new SessionStore();
    sessions.rememberTaskBrief('c', target, '/tmp', state, { sentFull: true });
    expect(sessions.serializeTaskBriefs()).toEqual({});
  });

  it('ignores what does not look like a brief', () => {
    const sessions = new SessionStore();
    sessions.restoreTaskBriefs({ a: null, b: { state: { x: 1 }, sinceFull: 0 }, c: { state: {}, sinceFull: -1 } });
    expect(sessions.serializeTaskBriefs()).toEqual({});
  });
});

describe('a long section sends only what moved inside it', () => {
  const section = (body: string, delta?: BriefSection['delta']): BriefSection => ({ id: 's', title: 'S', body, ...(delta ? { delta } : {}) });

  it('uses the section\'s own delta when it is shorter', () => {
    const next = section('x'.repeat(500), () => 'just the change');
    const out = briefDelta({ s: 'old' }, [next]);
    expect(out.text).toContain('just the change');
    expect(out.text).not.toContain('x'.repeat(100));
    expect(out.state.s).toBe(next.body);
  });

  it('falls back to the whole body when the delta is no shorter or cannot say it', () => {
    expect(briefDelta({ s: 'old' }, [section('short', () => 'a much longer delta text')]).text).toContain('short');
    expect(briefDelta({ s: 'old' }, [section('whole body', () => undefined)]).text).toContain('whole body');
  });
});
...
describe('mcp scope', () => {
  ...
  it('gives an unscoped server to everyone', () => {
    expect(Object.keys(serversFor('copilot', servers))).toEqual(['everywhere']);
...
describe('mcp profiles forget what was taken away', () => {
  const profile = (): AccountProfile & { homeDir: string } => ({
    ...
    hasSecret: false,
    priority: 1,
    homeDir: mkdtempSync(join(tmpdir(), 'cortex-mcp-')),
  });
