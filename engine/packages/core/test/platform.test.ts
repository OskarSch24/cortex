...
import { shareCodexPluginHost } from '../src/adapters/codex.js';
...
const tmp = () => mkdtempSync(join(tmpdir(), 'cortex-test-'));
...
describe('mcp sync', () => {
  ...
  it('writes each provider its native config shape', () => {
    ...
    expect(toml).toContain('cortex-mcp-begin');
  });
  ...
  it('re-syncing replaces the managed codex block instead of appending', () => {
    ...
    expect(toml.match(/cortex-mcp-begin/g)).toHaveLength(1);
    expect(toml).toContain('model = "x"');
  });

  it('entfernt eine doppelte grok-mcp_servers-Tabelle hinter dem managed-Block', () => {
    const home = tmp();
    mkdirSync(join(home, '.grok'), { recursive: true });
    writeFileSync(
      join(home, '.grok', 'config.toml'),
      'model = "keep"\n\n# cortex-mcp-begin (managed — do not edit inside)\n[mcp_servers.ctx7]\ncommand = "npx"\n# cortex-mcp-end\n\n[mcp_servers.ctx7]\ncommand = "npx"\n',
    );
    const error = syncMcpToProfile(
      { id: 'x', provider: 'grok', label: 'l', authMode: 'managed-home', homeDir: home, hasSecret: false, priority: 1 },
      servers,
    );
    expect(error).toBeUndefined();
    const toml = readFileSync(join(home, '.grok', 'config.toml'), 'utf8');
    expect(toml.match(/\[mcp_servers\.ctx7\]/g)).toHaveLength(1);
    expect(toml).toContain('model = "keep"');
  });
...
describe('spawning a CLI that is a JavaScript file', () => {
  ...
  it('says which CLI is missing when there is no such binary', async () => {
    ...
    for await (const ev of spawnLines('cortex-no-such-cli', [], {
      cwd: tmp(),
      env: process.env,
      signal: new AbortController().signal,
    })) {
...
describe('codex plugin host', () => {
  it('links the shared code-mode host into a profile without copying it', () => {
    const home = tmp();
    const shared = join(homedir(), '.codex', 'plugins', '.plugin-appserver');
    shareCodexPluginHost(home);
    const target = join(home, 'plugins', '.plugin-appserver');
    if (!existsSync(shared)) {
      // Ohne installierte Codex-Plugins gibt es nichts zu verlinken — und das
      // Fehlen darf nicht zum Absturz führen.
      expect(existsSync(target)).toBe(false);
      return;
    }
    expect(lstatSync(target).isSymbolicLink()).toBe(true);
    expect(existsSync(join(target, 'codex-code-mode-host'))).toBe(true);
  });

  it('leaves an existing entry alone and never throws without a home', () => {
    expect(() => shareCodexPluginHost(undefined)).not.toThrow();
    const home = tmp();
    mkdirSync(join(home, 'plugins', '.plugin-appserver'), { recursive: true });
    shareCodexPluginHost(home);
    expect(lstatSync(join(home, 'plugins', '.plugin-appserver')).isDirectory()).toBe(true);
  });
});

describe('mcp for grok', () => {
  const profile = (over: Partial<AccountProfile>): AccountProfile => ({
    id: 'x', provider: 'claude', label: 'l', authMode: 'managed-home', hasSecret: false, priority: 1, ...over,
  });

  it('writes the managed block into .grok/config.toml', () => {
    const home = tmp();
    const parsed = parseMcpFile(JSON.stringify({ servers: { ctx7: { command: 'npx', args: ['ctx7'] } } }));
  ...
  // über mcp-remote hätte bei einer 401 von selbst einen Browser geöffnet.
  it('gives grok the url itself — no mcp-remote detour', () => {
    const home = tmp();
    const parsed = parseMcpFile(JSON.stringify({ servers: { docs: { url: 'https://mcp.example.com/mcp' } } }));
    if (!parsed.ok) throw new Error(parsed.error);
    syncMcpToProfile(profile({ provider: 'grok', homeDir: home }), parsed.servers);
    const toml = readFileSync(join(home, '.grok', 'config.toml'), 'utf8');
