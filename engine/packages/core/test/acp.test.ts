...
/**
 * The ACP path (Grok + Copilot) verified against a fake agent that speaks
...
describe('ACP adapter', () => {
  ...
  it('answers permission requests from the mode, silently, when not asking', async () => {
    ...
    for (const events of [granted, denied]) {
      ...
      expect(events.some((e) => e.type === 'deferred-instruction')).toBe(false);
    }
  });
  ...
  it('delivers a mid-run message and answers it as its own turn', async () => {
    ...
    for await (const ev of run('SLOW essay', { handle })) {
      ...
      if (!injected && ev.type === 'text-delta' && handle.inject) {
        injected = await handle.inject('INJECTED follow-up');
      }
    }
  ...
  it('does not stream the replayed history of a resumed session as the new answer', async () => {
    const events = await collect(run('follow-up', { resumeSessionId: 'fake-session-1' }));
    const said = events
      .filter((e) => e.type === 'text-delta')
      .map((e) => (e as { text: string }).text)
      .join('');
    expect(said).toBe('done');
    expect(said).not.toContain('EARLIER');
  ...
  it('reports a missing CLI as an error instead of hanging', async () => {
    const events = await collect(
      runAcp({
        command: '/nonexistent/cortex-acp-agent',
        args: [],
        env: process.env,
