...
describe('task brief', () => {
  ...
  it('never repeats a convention file the CLI already reads', () => {
    ...
    expect(readsNatively('copilot', 'CLAUDE.md')).toBe(false);
    expect(buildBrief({ provider: 'claude', conventions })).not.toContain('house rules');
    expect(buildBrief({ provider: 'copilot', conventions })).toContain('house rules');
  });
...
describe('provider brief', () => {
  it('gives the thinner CLIs the instructions Claude does not need', () => {
    ...
    const copilot = briefLinesFor({ provider: 'copilot', permissionMode: 'edits' });
    expect(copilot.length).toBeGreaterThan(claude.length);
    expect(copilot.some((l) => l.id === 'copilot-read-first')).toBe(true);
    expect(claude.some((l) => l.id.startsWith('copilot-'))).toBe(false);
  });
  ...
  it('drops a line the evidence disqualified', () => {
    const lines = briefLinesFor({
      provider: 'copilot',
      permissionMode: 'edits',
      disabledLineIds: ['copilot-full-answer'],
    });
    expect(lines.some((l) => l.id === 'copilot-full-answer')).toBe(false);
  });
});
...
describe('location', () => {
  it('names the configured home and forbids IP lookups', async () => {
    const { locationBrief } = await import('../src/context/locationBrief.js');
    expect(locationBrief('Hamburg')).toContain('The user is based in Hamburg');
    expect(locationBrief('Hamburg')).toContain('Never determine the location from the IP address');
    expect(locationBrief('  ')).toContain('You do not know where the user is');
    expect(locationBrief(undefined)).toContain('ask once');
  });
});
