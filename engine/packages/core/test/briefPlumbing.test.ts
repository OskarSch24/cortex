...
describe('brief plumbing', () => {
  it('preserves denial feedback queued behind stale events when a user immediately stops', async () => {
    const fake = new FakeAdapter('claude');
    const controller = new AbortController();
    fake.run = async function* () {
      controller.abort();
      yield { type: 'text-delta' as const, text: 'stale work' };
      yield { type: 'deferred-instruction' as const, text: 'Do not delete files.' };
      yield { type: 'result' as const, text: 'stale result' };
    };
    const { orchestrator, sessions } = setup(fake);
    const events = await drain(orchestrator.run(task(), controller.signal));
    expect(events.filter(event => event.type === 'deferred-instruction')).toEqual([{ type: 'deferred-instruction', text: 'Do not delete files.' }]);
    expect(events.some(event => ['text-delta', 'result', 'chain-exhausted'].includes(event.type))).toBe(false);
    expect(sessions.getHistory('conv1')).toEqual([]);
  });

  it('delivers deferred denial reasons on the next normal turn without changing recorded user text', async () => {
    const fake = new FakeAdapter('claude');
    fake.script('claude-a', [{ type: 'result', text: 'ok' }]);
    const { orchestrator, sessions } = setup(fake);
    await drain(orchestrator.run(task({ permissionNotes: ['Keine Datei löschen; erst den Inhalt zeigen.'] }), new AbortController().signal));
    expect(fake.runs[0]?.prompt).toContain('Keine Datei löschen; erst den Inhalt zeigen.');
    expect(fake.runs[0]?.prompt).toContain('rename this variable');
    expect(sessions.getHistory('conv1')[0]?.text).toBe('rename this variable');
  });

  it('passes the configured silence timeout through to the provider', async () => {
    const fake = new FakeAdapter('claude');
    fake.script('claude-a', [{ type: 'result', text: 'ok' }]);
    const { orchestrator } = setup(fake);
    await drain(orchestrator.run(task({ idleTimeoutMs: 420_000 }), new AbortController().signal));
    expect(fake.runs[0]?.idleTimeoutMs).toBe(420_000);
  });
