...
export interface BriefSection {
  ...
  body: string;
  /**
   * What to send instead of the whole body to a session that was told
   * `previous` — for sections that are long and change a little at a time.
   * Undefined when the full body says it better.
   */
  delta?: (previous: string) => string | undefined;
}
...
export function briefDelta(previous: BriefState | undefined, sections: BriefSection[]): BriefDelta {
  ...
  if (changed.length > 0) {
    parts.push(render(changed.map((s) => {
      const before = previous[s.id];
      const body = before !== undefined ? s.delta?.(before) : undefined;
      return body !== undefined && body.length < s.body.length ? { ...s, body } : s;
    })));
  }
  if (dropped.length > 0) {
...
function droppedTitle(id: string): string {
  ...
  const names: Record<string, string> = {
    ...
    notizen: 'the notes of this chat',
