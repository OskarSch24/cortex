...
const UNIVERSAL: BriefLine[] = [
  ...
  // sichtbaren Browser öffnen, statt die Tools des verbundenen Plugins zu nehmen.
  {
    ...
    text:
      'For data from a service with a connected MCP server or API (YouTube, Gmail, calendars, analytics), ' +
      "use that server's tools — including follow-up calls when a first result looks incomplete. Do not open " +
      'a browser to read what an API can return; if the tools cannot answer, say which data is missing.',
  },
];
