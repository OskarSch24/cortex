...
export interface ToolDetail {
  ...
  path?: string;
  action: ToolAction;
  /** Lines written and replaced, when the tool call says so — `+68 -0` in the transcript. */
  added?: number;
  removed?: number;
}
 ...
 *
 * Compared in NFC: macOS hands out folder paths decomposed ("o" + "¨") while
 * agents write them composed ("ö"). Without that, every file in a project with
 * an umlaut in its path fell through to "~/…" and the dock could not open it.
 */
export function shortenPath(path: string, cwd?: string): string {
  let out = path.normalize('NFC');
  const base = cwd?.normalize('NFC');
  if (base && out.startsWith(base)) out = out.slice(base.length).replace(/^[/\\]/, '');
...
export function describeToolUse(name: string, rawInput: unknown, cwd?: string): ToolDetail {
  ...
  if (tool === 'edit' || tool === 'multiedit' || tool === 'str_replace' || tool === 'applypatch') {
    ...
    const counted = edits
      ? edits.reduce<{ added: number; removed: number }>((n, e) => ({ added: n.added + countLines(str(e, 'new_string')), removed: n.removed + countLines(str(e, 'old_string')) }), { added: 0, removed: 0 })
      : oldText !== undefined || newText !== undefined ? { added: countLines(newText), removed: countLines(oldText) } : {};
    return {
      action: 'edit',
      path: short,
