...
export interface LiveRunHandle {
  inject?: (text: string) => boolean | Promise<boolean>;
  /**
...
export interface RunRequest {
  ...
  coldPrompt?: string;
  /**
   * Bilder, die mit dieser Nachricht als Bild ans Modell gehen (attachments.ts).
   * Der Pfad steht zusätzlich im Text; wer keine Bilder annimmt, hat dann ihn.
   */
  images?: import('./attachments.js').ImageInput[];
  cwd: string;
  model?: string;
  resumeSessionId?: string;
  /**
   * Fork `resumeSessionId` at this checkpoint (see the `result` event) instead
   * of continuing it: the new session remembers everything up to that turn and
   * nothing after, and the original stays untouched. Set after the user went
   * back to an earlier message, edited one, or branched a chat off.
   */
  resumeAt?: string;
  ...
  /** Maximum silence from a provider before ending a stuck turn (default three minutes). */
 ...
 * - file: the auth artifact appears on disk (codex auth.json, grok auth.json)
...
export interface ProviderAdapter {
  ...
  readonly models: import('../models/catalog.js').ModelOption[];
